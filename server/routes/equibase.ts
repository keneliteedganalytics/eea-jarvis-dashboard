// Equibase ingest admin routes (behind the global HTTP basic auth in index.ts).
//
//   POST /api/admin/equibase/ingest  -> manual trigger.
//        body: { raceDate: "YYYY-MM-DD", trackCodes?: string[] }
//   GET  /api/admin/equibase/status  -> last run + enabled tracks.

import { Router } from "express";
import { z } from "zod";
import { ingestForDate, readConfig } from "../services/equibase-ingest";
...
export function equibaseAdminRouter(): Router {
  const router = Router();
