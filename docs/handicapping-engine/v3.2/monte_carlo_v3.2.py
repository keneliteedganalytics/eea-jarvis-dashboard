#!/usr/bin/env python3
"""
Elite Edge Analytics — Race Monte Carlo Simulator (v3.2 IN-DEV 2026-06-12)
...
# v3.2: track bias hooks (loaded via --bias-state json file or kwarg)
try:
    from track_bias_detector import bias_adjustment as _v32_bias_adj
    _V32_AVAILABLE = True
except Exception:
    _V32_AVAILABLE = False
...
def apply_bias_adjustments(horses_dict, bias_state):
    """v3.2: apply track-bias adjustments to each horse based on current bias state."""
    if not bias_state or not _V32_AVAILABLE or not bias_state.get("active"):
        return {p: 0.0 for p in horses_dict}
    out = {}
    for p, h in horses_dict.items():
        adj = _v32_bias_adj(h, bias_state)
        out[p] = adj["total"]
    return out
...
def run_monte_carlo(data, meltdown_prob, n_full, n_trials, seed):
    ...
    for p in bases:
        bases[p] += field_adj[p]
    # v3.2: track-bias adjustments (if bias_state present in data)
    bias_state = data.get("bias_state")
    bias_adj = apply_bias_adjustments(horses, bias_state)
    for p in bases:
        bases[p] += bias_adj[p]
    anchor = race["anchor_pgm"]
    print(f"\n[v3.1] field pace_end median = {field_pe_median:.2f}")
    if bias_state and bias_state.get("active"):
        print(f"[v3.2] bias active: hot_pps={bias_state.get('hot_pps')} dead_pps={bias_state.get('dead_pps')} style_bias={bias_state.get('style_bias')} confidence={bias_state.get('confidence')}")
        for p, a in bias_adj.items():
            if a != 0:
                print(f"[v3.2] bias adj for #{p}: {a:+.2f}")
